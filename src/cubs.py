...
def main():
    mlb = mlbstatsapi.Mlb()
    cubs_schedule = mlb.get_scheduled_games_by_date(start_date="2025-03-25", end_date="2025-03-25", teamId=112, sportId=1)
    ...
    for item in cubs_schedule:
        ...
        if gamepk.teams.home.team.id == 112:
            print('got here')
            todays_score_api = f"https://statsapi.mlb.com/api/v1/schedule?gamePk={item.gamepk}"
